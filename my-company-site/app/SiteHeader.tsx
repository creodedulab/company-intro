...
export default function SiteHeader() {
  ...
  return (
    <>
      {/* 상단 네비게이션 바 */}
      <nav className="fixed top-0 left-0 right-0 h-20 bg-white/90 backdrop-blur-md shadow-sm z-[100] flex items-center justify-between px-6 md:px-10 border-b border-slate-100">
      ...
      {isMenuOpen && (
        <>
          {/* 1. 투명 배경막 (클릭 시 닫힘)
            메뉴판 아래의 '빈 공간' 역할을 합니다. 
            z-98로 설정하여 메뉴판(z-99)보다는 아래, 본문보다는 위에 위치합니다.
          */}
          <div 
          ...
          {/* 2. 실제 메뉴판 (반투명 효과)
            bg-white/90 : 흰색 배경인데 90% 불투명 (살짝 비침)
            backdrop-blur-md : 뒤에 있는 내용이 흐릿하게 보이는 유리 효과
          */}
          <div className="fixed top-20 left-0 right-0 bg-white/90 backdrop-blur-md border-b border-slate-200 shadow-xl md:hidden z-[99] animate-slideDown">
